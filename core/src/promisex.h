...
public:
    ...
    static shared_ptr<Promise> from(shared_ptr<promise<void>> p);
    static shared_ptr<Promise> from(function<any(bool*)>&& task);
...
public:
    ...
    bool isBusy();
public:
    void then(PromiseThen&& handler, bool runOnMainThread = true);
